...
export const isAlertModuleAvailable = async (): Promise<boolean> => {
  try {
    if (
      NativeModules.AlertModule &&
      NativeModules.AlertModule.isUsable
    ) {
    ...
    console.error('[isAlertModuleAvailable] AlertModule has NOT loaded, Error:', err);
  }
  return false;
};
...
export const createAlert = async (message: string): Promise<boolean> => {
  try {
    if (
      NativeModules.AlertModule &&
      NativeModules.AlertModule.showToast
    ) {
      const isAvailable: boolean = await NativeModules.AlertModule.showToast(message);
      return isAvailable;
    }
